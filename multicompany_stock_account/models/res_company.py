...
class ResCompany(models.Model):
    ...
    def _default_stock_properties(self):
        ...
        self.default_stock_journal = ir_property_obj.with_context(force_company=self.id).get(
            'property_stock_journal', 'product.category')
        self.default_stock_account_input_categ_id = ir_property_obj.with_context(force_company=self.id).get(
            'property_stock_account_input_categ_id', 'product.category')
        self.default_stock_account_output_categ_id = ir_property_obj.with_context(force_company=self.id).get(
            'property_stock_account_output_categ_id', 'product.category')
        self.default_stock_valuation_account_id = ir_property_obj.with_context(force_company=self.id).get(
