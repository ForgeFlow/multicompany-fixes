...
class Base(models.AbstractModel):
    ...
    def add_company_suffix(self, names):
        ...
        if multicompany_group not in self.env.user.groups_id or self._context.get(
            "not_display_company"
        ):
            return names
        for name in names:
            ...
            name = (
                "{} [{}]".format(name[1], rec.company_id.sudo().name)
                if rec.company_id
                else name[1]
            )
            res += [(rec.id, name)]
    ...
    def check_company(self, company_id):
        ...
        if "company_id" not in self._fields:
            return True
        return not bool(
            self.filtered(lambda r: r.company_id and r.company_id != company_id)
        )
                ...
                for fld in fields:
                    if not fld.check_company(rec.company_id):
                        raise ValidationError(
                            ...
                            % (rec._name, rec.display_name, fld._name, fld.display_name)
                        )
