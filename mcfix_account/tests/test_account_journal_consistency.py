...
class TestAccountJournalConsistency(TestAccountChartTemplate):
    ...
    def _create_bank(self):
        return self.env["res.bank"].create({"name": "bank 1"})
    ...
    def test_journal_company_consistency(self):
        ...
        with self.assertRaises(UserError):
            self.cash_journal.write({"profit_account_id": self.account_2.id})
        self.cash_journal.profit_account_id = False

        with self.assertRaises(UserError):
            self.cash_journal.write({"loss_account_id": self.account_2.id})
        self.cash_journal.loss_account_id = False

        with self.assertRaises(UserError):
            self.cash_journal.write({"default_debit_account_id": self.account_2.id})
        self.cash_journal.default_debit_account_id = False

        with self.assertRaises(UserError):
            self.cash_journal.write({"default_credit_account_id": self.account_2.id})
        ...
        with self.assertRaises(UserError):
            self.cash_journal.write({"company_id": self.company_2.id})
        ...
        with self.assertRaises(UserError):
            journal.bank_account_id = bank_account.id
