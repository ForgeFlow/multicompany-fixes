...
class PosConfig(models.Model):
    ...
    pricelist_id = fields.Many2one(check_company=True, default=_default_pricelist)
    ...
    def write(self, vals):
        for pos_config in self:
            if vals.get("company_id") and not vals.get("sequence_id", False):
                sequence_id = vals.get("sequence_id", pos_config.sequence_id.id)
                sequence = self.env["ir.sequence"].browse(sequence_id)
                if sequence and sequence.company_id.id != vals["company_id"]:
                    sequence.with_context(bypass_company_validation=True).sudo().write(
                        {"company_id": vals["company_id"]}
                    )
            if vals.get("company_id") and not vals.get("sequence_line_id", False):
                sequence_id = vals.get(
                    "sequence_line_id", pos_config.sequence_line_id.id
                )
                sequence = self.env["ir.sequence"].browse(sequence_id)
                if sequence and sequence.company_id.id != vals["company_id"]:
                    sequence.with_context(bypass_company_validation=True).sudo().write(
    ...
    def _onchange_company_id(self):
        if not self.journal_id or not self.journal_id.check_company(self.company_id):
            self.journal_id = self.with_context(
                company_id=self.company_id.id
            )._default_sale_journal()
        if not self.invoice_journal_id or not self.invoice_journal_id.check_company(
            self.company_id
        ):
