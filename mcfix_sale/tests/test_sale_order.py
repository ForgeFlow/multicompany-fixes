...
class TestSaleOrder(TransactionCase):
    ...
    def setUp(self):
        ...
        manager_sale_test_group = self.create_full_access(
            ['sale.order'])
    ...
    def create_full_access(self, list_of_models):
        manager_sale_test_group = self.env['res.groups'].sudo().create({
            'name': 'group_manager_sale_test'
        })
    ...
    def test_onchanges(self):
        self.so_1 = self.so_model.sudo(self.user).new({
            'name': 'Sale - Test',
    ...
    def test_constrains(self):
        self.so_1 = self.so_model.sudo(self.user).create({
            'name': 'Sale - Test',
            'partner_id': self.partner.id,
            'pricelist_id': self.pricelist.id,
            'company_id': self.company.id,
