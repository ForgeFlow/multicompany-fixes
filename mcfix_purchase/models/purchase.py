    ...
    def onchange_partner_id(self):
        if not self.partner_id:
            ...
            self.payment_term_id = self.partner_id.with_context(
                force_company=self.company_id.id).property_supplier_payment_term_id.id
            self.currency_id = self.partner_id.with_context(
                force_company=self.company_id.id).property_purchase_currency_id.id or self.env.user.company_id.currency_id.id
...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_product_id(self):
        ...
        fpos = self.order_id.fiscal_position_id or self.order_id.with_context(
            force_company=self.company_id.id).partner_id.property_account_position_id
        taxes = self.product_id.supplier_taxes_id.filtered(
            lambda r: not self.company_id or r.company_id == self.company_id)
        self.taxes_id = fpos.map_tax(taxes, self.product_id, self.order_id.partner_id) if fpos else taxes
