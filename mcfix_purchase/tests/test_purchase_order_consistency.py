...
class TestPurchaseOrderConsistency(TestAccountChartTemplate):
    ...
    def setUp(self):
        ...
        self.product2.write(
            {"responsible_id": self.user.id, "company_id": self.company_2.id}
        )
        ...
        self.cash_journal = (
            self.journal_model.with_user(self.user)
            .sudo()
            .create(
                {
        ...
        self.purchase1 = self.with_context(company_id=self.company.id)._create_purchase(
            self.company, self.product1, self.tax_1, self.partner_1
        )

        self.purchase1.button_confirm()
        self.invoice = self._create_invoice(
            self.purchase1, self.partner_1, self.purchase_journal, self.cash_account_id
        )
