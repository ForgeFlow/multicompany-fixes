...
class ResCompany(models.Model):
    ...
    transfer_account_id = fields.Many2one(
        domain=lambda self: (
            ...
            "('reconcile', '=', True),"
            "('user_type_id.id', '=',"
            + str(self.env.ref("account.data_account_type_current_assets").id)
            + "),"
            "('company_id', '=', active_id),"
            "('deprecated', '=', False)]"
        ),
    ...
    def get_property_value(self, model, field):
        value = (
            ...
            .with_context(force_company=self.id)
            .sudo()
            .get(field, model)
        )
        if value:
    ...
    def _compute_tax(self):
        ...
        for record in self:
            record.default_sale_tax_id = tax_obj.browse(
                IrDefault.get("product.template", "taxes_id", company_id=record.id)
            )
            record.default_purchase_tax_id = tax_obj.browse(
                IrDefault.get(
    ...
    def _inverse_tax(self):
        ...
        for record in self:
            IrDefault.set(
                "product.template",
                "taxes_id",
                record.default_sale_tax_id.ids,
                company_id=record.id,
            )
            IrDefault.set(
                "product.template",
                "supplier_taxes_id",
                record.default_purchase_tax_id.ids,
                company_id=record.id,
            )
